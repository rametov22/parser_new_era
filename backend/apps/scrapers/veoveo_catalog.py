...
class VeoVeoCatalogClient:
    ...
    def get_details_page(self, *, page: int, page_size: int) -> VeoVeoCatalogPage:
        payload = {
            "pagination": {
                "page": page,
                "pageSize": page_size,
        ...
        try:
            ...
            raise VeoVeoCatalogError(f"VeoVeo API request failed{suffix}") from exc
        ...
        if total and not items and page == 1:
            raise VeoVeoCatalogDataError(
...
def _required_non_negative_int(value: Any, field_name: str) -> int:
    ...
    if number is None:
        raise VeoVeoCatalogDataError(f"{field_name} must be a non-negative integer")
    return number
